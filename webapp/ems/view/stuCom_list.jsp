...
<div class="row alert alert-info" style="margin:0px;padding:5px;">
    <form action="/comm/list.do" method="post" class="form-horizontal">
    ...
    <input type="button" class="btn btn-success" value="Add" onclick=
            "javascript:window.location='/comm/loadAdd.do'"/>
    </form>
</div>
...
<div class="row" style="padding:0px; padding-left: 15px; padding-right: 0px ;">
    <div class="row" style="padding:0px;padding-left: 15px;padding-right: 15px;" align="right">
        <display:table class="table table-condensed table-striped" name="list"
                       pagesize="10" requestURI="/comm/list.do">
            <display:column property="comId" title="Id"/>
            <display:column property="stuName" title="Student"/>
            <display:column property="staffName" title="Communicator"/>
            <display:column property="comContent" title="Content"/>
            <display:column property="comTime" format="{0,date,yyyy-MM-dd}" title="Date"/>
            <display:column href="/comm/load.do?comId=${comm.comId}"
                            paramId="comId" paramProperty="comId" value="edit" title="edit"/>
            <display:column href="/comm/delete.do?classroomId=${comm.comId}"
                            paramId="comId" paramProperty="comId" value="delete" title="delete"/>
        </display:table>
    </div>
