...
<div class="row alert alert-info" style="margin:0px;padding:5px;">
    <form action="/template/list.do" method="post" class="form-horizontal">
    <div class="col-sm-1">Title:</div>
        <div class="col-sm-3">
            <input type="text" name="templateTitle" class="form-control input-sm"/>
        </div>
    <input type="submit" class="btn btn-danger " value="Search"/>
    <input type="button" class="btn btn-success" value="Add Template" onclick=
            "javascript:window.location='ems/view/template_add.jsp'"/>
    </form>
</div>
...
<div class="row" style="padding:0px;padding-left: 15px;padding-right: 15px;" align="right">
    <display:table class="table table-condensed table-striped" name="list"
                   pagesize="10" requestURI="template/list.do" >
        <display:column property="templateId" title="Id"/>
        <display:column property="templateTitle" title="Title"/>
        <display:column property="templateContent" title="Content"/>
        <display:column property="templateEngine" title="Status"/>
        <display:column href="/template/load.do?templateId=${tinfo.templateId}"
                        paramId="templateId" paramProperty="templateId" value="edit" title="edit"/>
        <display:column href="/template/delete.do?templateId=${tinfo.templateId}"
                        paramId="templateId" paramProperty="templateId" value="delete" title="delete"/>
    </display:table>
</div>
</body>
</html>