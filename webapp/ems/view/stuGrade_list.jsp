...
<div class="row alert alert-info" style="margin:0px;padding:5px;">
    <form action="/grade/list.do" method="post" class="form-horizontal">
    ...
    <input type="button" class="btn btn-success" value="Add" onclick=
            "javascript:window.location='/grade/loadAdd.do'"/>
    </form>
</div>
...
<div class="row" style="padding:0px; padding-left: 15px; padding-right: 0px ;">
    <div class="row" style="padding:0px;padding-left: 15px;padding-right: 15px;" align="right">
        <display:table class="table table-condensed table-striped" name="list"
                       pagesize="10" requestURI="/grade/list.do">
            <display:column property="stuGradeId" title="Id"/>
            <display:column property="stuName" title="Student"/>
            <display:column property="staffName" title="Manager"/>
            <display:column property="stuGradeSub" title="Subject"/>
            <display:column property="stuGrade" title="Grade"/>
            <display:column href="/grade/load.do?stuGradeId=${grade.stuGradeId}"
                            paramId="stuGradeId" paramProperty="stuGradeId" value="edit" title="edit"/>
            <display:column href="/grade/delete.do?classroomId=${grade.stuGradeId}"
                            paramId="stuGradeId" paramProperty="stuGradeId" value="delete" title="delete"/>
        </display:table>
    </div>
